...
def _styled(text: str) -> str:
    ...
    while i < len(text):
        ch = text[i]
        if ch == '<':
            in_tag = True; result.append(ch)
        elif ch == '>':
            in_tag = False; result.append(ch)
        elif in_tag:
            result.append(ch)
        else:
...
def _get_cached_poster(title: str, template: str) -> Optional[Dict]:
    ...
    try:
        ...
        row = _pg_exec(
            ...
            "FROM poster_cache WHERE cache_key = %s", (key,)
        )
        if row:
...
def _join_btn_text() -> str:
    return _styled_plain(_setting("env_JOIN_BTN_TEXT", JOIN_BTN_TEXT_DEFAULT))

def _here_link_text() -> str:
    return _styled(_setting("env_HERE_IS_LINK_TEXT", HERE_LINK_TEXT_DEFAULT))

def _link_expired_text() -> str:
    return _styled_plain(_setting("env_LINK_EXPIRED_TEXT", LINK_EXPIRED_TEXT_DEFAULT))
        ...
        for ch_data in channels:
            try:
                ...
                inv = await bot.create_chat_invite_link(
                    chat_id=cid_int, expire_date=expire_ts, member_limit=1,
                    name=f"Filter-{int(time.time())}", creates_join_request=False,
                )
...
def _apply_text_watermark_layer(img: Any, text: str, position: str,
    ...
    try:
        ...
        draw.text((x + 2, y + 2), text, font=fnt, fill=(0, 0, 0, min(180, opacity)))
        ...
        draw2.text((x, y), text, font=fnt, fill=_parse_color(color, opacity))
        return Image.alpha_composite(img, layer)
    except Exception as exc:
        logger.debug(f"text watermark layer error: {exc}")
        return img

def _apply_image_watermark_layer(img: Any, overlay: Any, position: str,
                                   scale: float = 0.12, opacity: int = 200) -> Any:
    ...
    try:
        ...
        overlay_rs = overlay.resize((ow, oh), Image.LANCZOS).convert("RGBA")
        if opacity < 255:
...
async def apply_watermark_layers(img: Any, chat_id: int, bot: Any = None) -> Any:
    if not PIL_OK:
        return img
    la = get_wm_layer(chat_id, "a")
    if la.get("enabled", True) and la.get("text"):
        img = _apply_text_watermark_layer(img, text=la["text"],
            position=la.get("position", "bottom-right"),
            font_size=int(la.get("font_size", 24)),
            color=la.get("color", "#FFFFFF"), opacity=int(la.get("opacity", 150)))
    lb = get_wm_layer(chat_id, "b")
    if lb.get("enabled") and lb.get("text"):
        img = _apply_text_watermark_layer(img, text=lb["text"],
            position=lb.get("position", "top-right"),
            font_size=int(lb.get("font_size", 18)),
            color=lb.get("color", "#AAAAAA"), opacity=int(lb.get("opacity", 120)))
...
async def _fetch_data(title: str, media_type: str) -> Optional[Dict]:
    ...
    try:
        ...
        fn = {"ANIME": _anilist_anime, "MANGA": _anilist_manga,
              "MOVIE": _tmdb_movie, "TV": _tmdb_tv}.get(media_type, _anilist_anime)
        return await loop.run_in_executor(None, fn, title)
    except Exception as exc:
        logger.debug(f"_fetch_data error: {exc}")
        return None

async def _generate_poster_data(title: str, template: str, media_type: str,
                                  chat_id: int, bot: Any = None,
                                  ) -> Tuple[Optional[BytesIO], str, Optional[Dict]]:
    loop = asyncio.get_event_loop()
    from poster_engine import (_make_poster, _build_anime_data, _build_manga_data,
                                _build_movie_data, _build_tv_data, _get_settings)
    data = await _fetch_data(title, media_type)
    if not data:
        return None, "", None
    build_fn = {"ANIME": _build_anime_data, "MANGA": _build_manga_data,
                "MOVIE": _build_movie_data, "TV": _build_tv_data}.get(media_type, _build_anime_data)
    ...
    poster_buf = await loop.run_in_executor(None, lambda: _make_poster(
    ...
    if poster_buf and PIL_OK:
        try:
            ...
            logger.debug(f"watermark compositing error: {exc}")
            poster_buf.seek(0)
...
async def _save_to_poster_channel(bot: Any, photo_buf: BytesIO, caption: str,
                                   template: str) -> Optional[Tuple[str, int]]:
...
async def _deliver_poster_mode(bot: Any, chat_id: int, title: str, template: str,
    ...
    try:
        poster_buf, caption, data = await asyncio.wait_for(
            _generate_poster_data(title, template, media_type, chat_id, bot=bot), timeout=30.0)
    ...
    if poster_buf:
        poster_buf.seek(0)
        try:
            sent_msg = await bot.send_photo(chat_id=chat_id, photo=poster_buf, caption=caption,
    ...
    if not sent_msg:
        try:
            sent_msg = await bot.send_message(chat_id=chat_id, text=caption, parse_mode="HTML",
                                               reply_markup=kb, reply_to_message_id=reply_to,
    ...
    if auto_delete_seconds > 0:
        asyncio.create_task(_auto_delete(bot, chat_id, sent_msg.message_id, delay=auto_delete_seconds))
    file_id = None
    channel_msg_id = 0
    ...
    if file_id:
        _save_poster_cache(title=title, template=template, file_id=file_id,
...
# ─────────────────────────────────────────────────────────────────────────────
#  DB LOOKUP — FIXED
#  Key fix: minimum 3 chars required → blocks single-letter false positives
#  Word-boundary match only applied when query >= 4 chars
# ─────────────────────────────────────────────────────────────────────────────

def _find_anime_in_db_sync(lower_text: str):
    """
    Exact / boundary match. Returns (title, channel_id, link_id, has_hindi).
    Requires >= 3 chars — single/double chars always return no-match.
    """
    if not lower_text or len(lower_text.strip()) < 3:
    ...
    try:
        ...
        for row in all_links:
            ...
            if a_title == lower_text:
                is_match = True
            elif len(lower_text) >= 4 and re.search(r'\b' + re.escape(a_title) + r'\b', lower_text):
                is_match = True
            elif len(lower_text) >= 4 and re.search(r'\b' + re.escape(lower_text) + r'\b', a_title):
                is_match = True
            elif len(lower_text) >= 4:
            ...
            try:
                ...
                for arow in acl:
                    ...
                    if an_lower == lower_text:
                        is_match = True
                    elif len(lower_text) >= 4 and re.search(r'\b' + re.escape(an_lower) + r'\b', lower_text):
                        is_match = True
...
def _find_anime_by_prefix_sync(first_word: str) -> list:
    """
    Prefix match: find all titles that START WITH first_word.
    Requires first_word >= 3 chars. Returns up to 5 results.
    """
    ...
    try:
        ...
        for row in (get_all_links(limit=1000, offset=0) or []):
            ...
            if norm in seen:
                continue
            if norm.startswith(first_word):
                seen.add(norm)
                is_hin = 'hindi' in norm or re.search(r'\bhin\b', norm) is not None
                results.append((ch_title, row[1], row[0], is_hin))
            if len(results) >= 5:
                return results
        ...
        for arow in (get_all_anime_channel_links() or []):
            ...
            if an_lower in seen:
                continue
            if an_lower.startswith(first_word):
                seen.add(an_lower)
                is_hin = 'hindi' in (arow[3] or "").lower() or 'hindi' in an_lower
                results.append((an_title, arow[2], arow[4], is_hin))
            if len(results) >= 5:
                break

    except Exception as exc:
        logger.debug(f"_find_anime_by_prefix_sync error: {exc}")
...
async def get_or_generate_poster(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    Trigger order:
      1. Full-text exact/boundary match (original behaviour)
      2. First-word prefix match — shows inline picker when ambiguous
    Requires >= 3 chars — "N" and "Na" are always silently ignored.
    """
    ...
    # ── Single letter → alphabet index browser ───────────────────────────────
    if len(text) == 1 and text.isalpha():
        await _send_index_page(context.bot, chat_id, text.lower(), page=0,
                               reply_to=reply_to)
        return

    # Block 2-char inputs (too ambiguous)
    if len(text) <= 2:
        return
    ...
    # Step 1: Full-text lookup
    matched_anime, matched_channel_id, matched_link_id, has_hindi = (
        await loop.run_in_executor(None, _find_anime_in_db_sync, lower)
    )

    # Step 2: First-word prefix fallback
    if not matched_anime:
        first_word = lower.split()[0] if lower.split() else ""
        if len(first_word) >= 3:
            prefix_matches = await loop.run_in_executor(
                None, _find_anime_by_prefix_sync, first_word
            )
            if len(prefix_matches) == 1:
                matched_anime      = prefix_matches[0][0]
                matched_channel_id = prefix_matches[0][1]
                matched_link_id    = prefix_matches[0][2]
                has_hindi          = prefix_matches[0][3]
            elif len(prefix_matches) > 1:
                buttons = [
                    [InlineKeyboardButton(
                        f"🎌 {row[0].title()}",
                        callback_data=f"filter_pick:{chat_id}:{row[0][:48]}"
                    )]
                    for row in prefix_matches[:5]
                ]
                buttons.append([
                    InlineKeyboardButton("❌ Cancel", callback_data=f"filter_pick_cancel:{chat_id}")
                ])
                try:
                    sel_msg = await context.bot.send_message(
                        chat_id=chat_id,
                        text=f"<b>Found {len(prefix_matches)} results</b> — pick one:",
                        parse_mode="HTML",
                    ...
                    if sel_msg:
                        asyncio.create_task(
                            _auto_delete(context.bot, chat_id, sel_msg.message_id, delay=60)
                        )
                except Exception as exc:
                    logger.debug(f"[filter] inline pick: {exc}")
                return
    ...
    if not matched_anime:
        ...
        if _hindi_kw and len(lower.strip()) >= 4:
            ...
            try:
                _req_kb = InlineKeyboardMarkup([[InlineKeyboardButton(
                ...
                _req_msg = await context.bot.send_message(
                    ...
                    parse_mode="HTML", reply_markup=_req_kb,
    ...
    raw_btn   = (_setting("env_JOIN_BTN_TEXT", "") or _setting("env_join_btn_text", "") or JOIN_BTN_TEXT_DEFAULT)
    ...
    if not data:
        kb = InlineKeyboardMarkup([[InlineKeyboardButton(
            _styled_plain("📩 Request"), callback_data=f"request_anime:{matched_anime[:48]}")]])
        try:
            sent = await bot.send_message(
                chat_id=chat_id,
                text=(f"<b>{_styled(html.escape(matched_anime))}</b>\n\n"
    ...
    if user_wants_hindi and not has_hindi:
        kb = InlineKeyboardMarkup([[
            ...
            InlineKeyboardButton(_styled_plain("📩 Request Hindi"),
    ...
    try:
        title_b, native, st, rows, desc, cover_url, score = await loop.run_in_executor(
            None, lambda: __import__('poster_engine')._build_anime_data(data))
        poster_buf = await loop.run_in_executor(
            None, lambda: __import__('poster_engine')._make_poster(
                template, title_b, native, st, rows, desc, cover_url, score,
                settings.get("watermark_text"), settings.get("watermark_position", "center"),
                None, "bottom"))
    ...
    if POSTER_DB_CHANNEL and poster_buf:
        try:
            poster_buf.seek(0)
            db_msg = await bot.send_photo(chat_id=POSTER_DB_CHANNEL, photo=poster_buf,
    ...
    if not sent_msg and data:
        ...
        if cover_direct:
            try:
                sent_msg = await bot.send_photo(
                    chat_id=chat_id, photo=cover_direct, caption=caption,
    ...
    if file_id_to_cache and _save_cache_fn:
        try:
            _save_cache_fn(cache_key=cache_key, anime_title=matched_anime, template=template,
                           file_id=file_id_to_cache, channel_id=matched_channel_id or 0,
                           caption=caption)
        except Exception:
            pass
...
# ─────────────────────────────────────────────────────────────────────────────
#  INLINE PICK CALLBACK
# ─────────────────────────────────────────────────────────────────────────────


# ─────────────────────────────────────────────────────────────────────────────
#  ALPHABET INDEX BROWSER
#  Type single letter → paginated inline list of all anime starting with it
#  Callbacks: idx_sel, idx_nav, idx_close
# ─────────────────────────────────────────────────────────────────────────────

_INDEX_PAGE_SIZE = 5  # anime per page


def _get_index_titles(letter: str) -> list:
    """Return sorted list of all anime titles starting with `letter` (lowercase)."""
    titles = []
    seen: set = set()
    ...
    try:
        ...
        for row in (get_all_links(limit=2000, offset=0) or []):
            ch_title = (row[2] or "").strip()
            if not ch_title:
                continue
            norm = _norm(ch_title)
        ...
        for arow in (get_all_anime_channel_links() or []):
            ...
            if an_lower in seen:
                continue
            if an_lower.startswith(letter):
                seen.add(an_lower)
                titles.append(an_title)

    except Exception as exc:
        logger.debug(f"_get_index_titles error: {exc}")

    return sorted(titles, key=lambda t: t.lower())


def _build_index_keyboard(letter: str, titles: list, page: int) -> InlineKeyboardMarkup:
    total   = len(titles)
    pages   = max(1, math.ceil(total / _INDEX_PAGE_SIZE))
    page    = max(0, min(page, pages - 1))
    start   = page * _INDEX_PAGE_SIZE
    chunk   = titles[start: start + _INDEX_PAGE_SIZE]

    rows = []
    for title in chunk:
        label = title.title() if len(title) <= 40 else title.title()[:38] + "…"
        # store: idx_sel:<letter>:<page>:<title_index_in_full_list>
        idx = titles.index(title)
        rows.append([InlineKeyboardButton(
            f"🎌 {label}",
            callback_data=f"idx_sel:{letter}:{idx}"
        )])
...
async def _send_index_page(bot: Any, chat_id: int, letter: str,
                            page: int = 0, reply_to: Optional[int] = None) -> None:
    loop = asyncio.get_event_loop()
    titles = await loop.run_in_executor(None, _get_index_titles, letter)

    if not titles:
        try:
            msg = await bot.send_message(
                chat_id=chat_id,
                text=f"😕 No anime found starting with <b>{letter.upper()}</b>",
                parse_mode="HTML",
                reply_to_message_id=reply_to,
            )
            if msg:
                asyncio.create_task(_auto_delete(bot, chat_id, msg.message_id, delay=30))
        except Exception:
            pass
        return

    kb   = _build_index_keyboard(letter, titles, page)
    _cnt = len(titles)
    text = f"🔤 <b>Anime starting with {letter.upper()}</b>\n<i>{_cnt} result" + ("s" if _cnt != 1 else "") + " found</i>"
    ...
    try:
        ...
        if msg:
            asyncio.create_task(_auto_delete(bot, chat_id, msg.message_id, delay=120))
    except Exception as exc:
        logger.debug(f"_send_index_page error: {exc}")


async def index_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handle idx_sel / idx_nav / idx_close / idx_noop callbacks."""
    ...
    if data.startswith("idx_nav:"):
        ...
        try:
            ...
            logger.debug(f"idx_nav edit: {exc}")
        return

    # ── Select anime ──────────────────────────────────────────────────────────
    if data.startswith("idx_sel:"):
        # idx_sel:<letter>:<title_index>
        parts = data.split(":")
        if len(parts) < 3:
            return
        letter = parts[1]
        ...
        loop   = asyncio.get_event_loop()
        titles = await loop.run_in_executor(None, _get_index_titles, letter)
        if idx >= len(titles) or idx < 0:
            return

        anime_title = titles[idx]

        # Delete the index menu
        try:
            await query.message.delete()
        except Exception:
            pass
...
async def filter_pick_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handle anime selection buttons from first-word multi-match picker."""
    query = update.callback_query
    if not query:
        return
    await query.answer()
...
def build_filter_poster_settings_keyboard(chat_id: int = 0) -> Any:
    ...
    rows = [
        [InlineKeyboardButton(("🟢 " if enabled else "🔴 ") + ("ON" if enabled else "OFF"),
                               callback_data=f"fp_toggle_{chat_id}"),
         InlineKeyboardButton(mode_label, callback_data=f"fp_mode_toggle_{chat_id}")],
        [_b("ani",   f"fp_tmpl_{chat_id}_ani"), _b("crun", f"fp_tmpl_{chat_id}_crun"), _b("net", f"fp_tmpl_{chat_id}_net")],
        [_b("dark",  f"fp_tmpl_{chat_id}_dark"), _b("light", f"fp_tmpl_{chat_id}_light"), _b("mod", f"fp_tmpl_{chat_id}_mod")],
        [_b("WM LAYER A", f"fp_wm_a_{chat_id}"), _b("WM LAYER B", f"fp_wm_b_{chat_id}"), _b("WM LAYER C", f"fp_wm_c_{chat_id}")],
        [_b("AUTO DEL", "fp_set_autodel"), _b("LINK EXPIRY", "fp_set_linkexpiry"), _b("♻️ CACHE", "fp_view_cache")],
        [_b("CLEAR CACHE", "fp_clear_cache"), _b("DB CHANNEL", "fp_channel_info")],
        [_b("✏️ JOIN BTN TEXT", f"fp_set_join_btn_{chat_id}"), _b("⏱ LINK EXPIRY", "fp_set_linkexpiry")],
        [InlineKeyboardButton("🎌 PRE-GEN ALL POSTERS", callback_data=f"fp_pregen_all_{chat_id}")],
        [InlineKeyboardButton("⏳ ANIM: " + ("ON ✅" if _get_anim_state() else "OFF 🔕"),
