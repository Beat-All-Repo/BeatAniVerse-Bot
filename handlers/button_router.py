...
async def button_handler(
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # autoupdate_* — Manga Auto-Update Tracker Panel
    # These 4 callbacks are generated by _show_autoupdate_menu() in
    # handlers/autoforward.py but had NO handler anywhere in the original repo.
    # ══════════════════════════════════════════════════════════════════════════

    if data == "autoupdate_add":
        if not is_admin:
            return
        # Alias → au_add_manga (same flow: search MangaDex then track)
        from core.state_machine import AU_ADD_MANGA_TITLE
        user_states[uid] = AU_ADD_MANGA_TITLE
        try:
            await query.delete_message()
        except Exception:
        ...
        await safe_send_message(
            context.bot, chat_id,
            b(small_caps("📚 track new manga")) + "\n\n"
            + bq(small_caps(
                "send the manga title to search on MangaDex:\n"
                "example: One Piece, Naruto, Berserk\n\n"
                "the bot will show matching results for you to confirm."
            )),
            reply_markup=InlineKeyboardMarkup([[
                bold_button("🔙 Cancel", callback_data="admin_autoupdate")
            ]]),
    ...
    if data == "autoupdate_remove":
        if not is_admin:
            return
        # Show list of tracked manga with stop buttons
        try:
            from api.mangadex import MangaTracker
            rows_rem = MangaTracker.get_all_tracked()
        except Exception:
            rows_rem = []
        if not rows_rem:
            await safe_answer(query, small_caps("no manga currently tracked."), show_alert=True)
            return
        text_rem = b(small_caps("🗑 remove manga tracking")) + "\n\n" + bq(
            small_caps("select a manga to stop tracking:")
        )
        kb_rem = []
        for rec in rows_rem[:20]:
            rec_id_r, manga_id_r, title_r = rec[0], rec[1], rec[2]
            target_r = rec[3] if len(rec) > 3 else ""
            label_r = f"🗑 {str(title_r)[:22]}"
            if target_r:
                label_r += f" → {str(target_r)[:10]}"
            kb_rem.append([bold_button(label_r, callback_data=f"au_stop_{manga_id_r}")])
        kb_rem.append([_back_btn("admin_autoupdate"), _close_btn()])
        await safe_edit_text(query, text_rem, reply_markup=InlineKeyboardMarkup(kb_rem))
        return

    if data == "autoupdate_run":
        if not is_admin:
            return
        await safe_answer(query, small_caps("▶️ running manga check now…"))
        try:
            ...
            tracked_run = MangaTracker.get_all_tracked()
            if not tracked_run:
                await safe_send_message(
                    context.bot, chat_id,
                    b(small_caps("⚠️ no manga tracked yet — add some first.")),
                    reply_markup=InlineKeyboardMarkup([[_back_btn("admin_autoupdate"), _close_btn()]]),
                )
                return

            progress_msg = await safe_send_message(
                context.bot, chat_id,
                b(small_caps(f"🔍 checking {len(tracked_run)} tracked manga…")),
            )
            ...
            for rec in tracked_run:
                rec_id_r2, manga_id_r2, manga_title_r2, target_chat_r2, lang_r2, last_chapter_r2, _ = rec
                try:
                    chapter_r2 = MangaDexClient.get_latest_chapter(manga_id_r2, lang_r2 or "en")
                    if not chapter_r2:
                        continue
                    attrs_r2 = chapter_r2.get("attributes", {}) or {}
                    ch_num_r2 = attrs_r2.get("chapter")
                    ch_id_r2  = chapter_r2.get("id", "")
                    if not ch_num_r2 or str(ch_num_r2) == str(last_chapter_r2):
                        continue
                    ch_info_r2 = MangaDexClient.format_chapter_info(chapter_r2)
                    text_notify = (
                        b("📚 New Chapter Released!") + "\n\n"
                        f"<b>Manga:</b> {b(e(str(manga_title_r2)))}\n\n"
                        + ch_info_r2 + "\n\n"
                        + bq(b("Enjoy reading! 🎉"))
                    )
                    kb_notify = [[
                        _IKB("📖 Read Now",  url=f"https://mangadex.org/chapter/{ch_id_r2}"),
                        _IKB("📚 Manga Page", url=f"https://mangadex.org/title/{manga_id_r2}"),
                    ]]
                    if target_chat_r2:
                        await safe_send_message(
                            context.bot, target_chat_r2, text_notify,
                            reply_markup=_IKM(kb_notify),
                        )
                    MangaTracker.update_last_chapter(rec_id_r2, ch_num_r2)
                    notified += 1
                    await _aio.sleep(0.5)
                except Exception as exc_r2:
                    logger.debug(f"autoupdate_run row {rec_id_r2} error: {exc_r2}")
                    errors += 1

            result_text = (
                b(small_caps("✅ manga check complete!")) + "\n\n"
                + bq(
                    f"<b>{small_caps('manga checked')}:</b> {len(tracked_run)}\n"
                    f"<b>{small_caps('new chapters found')}:</b> {notified}\n"
                    f"<b>{small_caps('errors')}:</b> {errors}"
                )
            )
            try:
                if progress_msg:
                ...
                await safe_send_message(context.bot, chat_id, result_text,
                    reply_markup=InlineKeyboardMarkup([[_back_btn("admin_autoupdate"), _close_btn()]]))

        except Exception as exc_run:
            logger.debug(f"autoupdate_run error: {exc_run}")
            await safe_send_message(
                context.bot, chat_id,
                b(small_caps(f"❌ run failed: {e(str(exc_run)[:80])}")),
                reply_markup=InlineKeyboardMarkup([[_back_btn("admin_autoupdate"), _close_btn()]]),
            )
        return

    if data == "autoupdate_settings":
        if not is_admin:
            return
        # Show manga tracking settings (language, interval, notification format)
        from database_dual import get_setting, set_setting
        notify_lang     = get_setting("manga_notify_language",  "en")  or "en"
        check_interval  = get_setting("manga_check_interval",   "10")  or "10"
        notify_fmt      = get_setting("manga_notify_format",    "full") or "full"
        notif_preview   = get_setting("manga_show_preview",     "true") == "true"

        text_aus = (
            b(small_caps("⚙️ manga auto-update settings")) + "\n\n"
            + bq(
                f"<b>{small_caps('notify language')}:</b> {code(notify_lang)}\n"
                f"<b>{small_caps('check interval')}:</b> {code(check_interval + ' min')}\n"
                f"<b>{small_caps('notify format')}:</b> {code(notify_fmt)}\n"
                f"<b>{small_caps('show cover preview')}:</b> {'✅' if notif_preview else '❌'}\n\n"
                + small_caps(
                    "language: en = english, ja = japanese, etc.\n"
                    "interval: how often to check for new chapters.\n"
                    "format: full = all info, compact = chapter only."
                )
            )
        )
        aus_langs = [("🇺🇸 en","en"),("🇯🇵 ja","ja"),("🇪🇸 es","es"),("🇧🇷 pt-br","pt-br")]
        lang_row = []
        for lang_lbl, lang_val in aus_langs:
            active = "✅ " if notify_lang == lang_val else ""
            lang_row.append(_btn(f"{active}{lang_lbl}", f"au_set_lang_{lang_val}"))
        aus_kb = [
            lang_row,
            [_btn(small_caps("5 min"), "au_set_interval_5"),
             _btn(small_caps("10 min"), "au_set_interval_10"),
             _btn(small_caps("30 min"), "au_set_interval_30")],
            [_btn(f"{'✅' if notify_fmt == 'full' else '  '} {small_caps('full format')}", "au_set_fmt_full"),
             _btn(f"{'✅' if notify_fmt == 'compact' else '  '} {small_caps('compact')}", "au_set_fmt_compact")],
            [_btn(f"{'✅' if notif_preview else '❌'} {small_caps('cover preview')}", "au_toggle_preview")],
            [_back_btn("admin_autoupdate"), _close_btn()],
        ]
        await safe_edit_text(query, text_aus, reply_markup=InlineKeyboardMarkup(aus_kb))
        return

    # ── autoupdate settings sub-actions ───────────────────────────────────────
    if data.startswith("au_set_lang_"):
        if not is_admin:
            return
        lang_v = data[len("au_set_lang_"):]
        from database_dual import set_setting
        set_setting("manga_notify_language", lang_v)
        await safe_answer(query, small_caps(f"✅ language set: {lang_v}"))
        await button_handler(update, context, "autoupdate_settings")
        return

    if data.startswith("au_set_interval_"):
        if not is_admin:
            return
        interval_v = data[len("au_set_interval_"):]
        from database_dual import set_setting
        set_setting("manga_check_interval", interval_v)
        await safe_answer(query, small_caps(f"✅ interval set: {interval_v} min"))
        await button_handler(update, context, "autoupdate_settings")
        return

    if data.startswith("au_set_fmt_"):
        if not is_admin:
            return
        fmt_v = data[len("au_set_fmt_"):]
        from database_dual import set_setting
        set_setting("manga_notify_format", fmt_v)
        await safe_answer(query, small_caps(f"✅ format set: {fmt_v}"))
        await button_handler(update, context, "autoupdate_settings")
        return

    if data == "au_toggle_preview":
        if not is_admin:
            return
        from database_dual import get_setting, set_setting
        cur_p = get_setting("manga_show_preview", "true")
        set_setting("manga_show_preview", "false" if cur_p == "true" else "true")
        await safe_answer(query, small_caps("✅ cover preview toggled"))
        await button_handler(update, context, "autoupdate_settings")
        return
